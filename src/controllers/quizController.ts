...
import { generateQuizSchema, updateQuizSchema } from '../utils/validation';
...
export async function generateQuiz(req: Request, res: Response) {
  ...
  const document = await StoredDocument.findOne({ _id: body.documentId, ownerId: req.user!.userId });
  if (!document) {
    throw new ApiError(404, 'Uploaded document not found or you do not own it. Upload it again.');
  }
  ...
  // The document is only needed while generating the quiz. Remove its binary
  // payload after successful generation to avoid accumulating large files in MongoDB.
...
export async function listMyQuizzes(req: Request, res: Response) {
  ...
  const withStats = await Promise.all(
    quizzes.map(async (quiz) => {
      ...
      return { ...quiz.toJSON(), attemptsCount };
    })
  );
...
export async function getQuiz(req: Request, res: Response) {
  ...
  const isOwner = req.user && quiz.creatorId.toString() === req.user.userId;
  ...
  // Never expose answer keys to participants before submission.
...
export async function updateQuiz(req: Request, res: Response) {
  ...
  if (body.questions) quiz.questions = body.questions as any;
...
function validateQuestionsForPublish(questions: any[]): string | null {
  for (let i = 0; i < questions.length; i += 1) {
    const q = questions[i];
    const label = `Question ${i + 1}`;
    if (!q?.questionText?.trim()) return `${label} has no question text`;
    if (!Number.isFinite(Number(q.marks)) || Number(q.marks) < 1 || Number(q.marks) > 5) {
      return `${label} must have marks between 1 and 5`;
    }

    if (q.type === 'mcq') {
      if (!Array.isArray(q.options) || q.options.length !== 4) return `${label} must have exactly 4 options`;
      if (typeof q.correctAnswer !== 'string' || !q.options.includes(q.correctAnswer)) {
        return `${label} has an invalid correct answer`;
      }
    } else if (q.type === 'true_false') {
      if (JSON.stringify(q.options) !== JSON.stringify(['True', 'False'])) {
      ...
      if (!Array.isArray(q.correctAnswer) || q.correctAnswer.length < 2 ||
          q.correctAnswer.some((answer: unknown) => !q.options.includes(answer))) {
        return `${label} must have at least 2 valid correct answers`;
      }
    } else if (q.type === 'short_answer') {
      if (!q.expectedAnswer?.trim()) return `${label} needs an expected answer`;
    } else {
...
export async function publishQuiz(req: Request, res: Response) {
  ...
  if (quiz.questions.length === 0) {
    throw new ApiError(400, 'Cannot publish a quiz with no questions');
  }
  const questionError = validateQuestionsForPublish(quiz.questions);
  if (questionError) throw new ApiError(400, questionError);
  ...
  res.json({
    quiz,
    shareLink: `${env.publicShareBaseUrl}/${quiz.shareCode}`,
  });
}

export async function closeQuiz(req: Request, res: Response) {
  const quiz = await Quiz.findById(req.params.id);
  if (!quiz) throw new ApiError(404, 'Quiz not found');
  if (quiz.creatorId.toString() !== req.user!.userId) {
    throw new ApiError(403, 'You do not own this quiz');
...
export async function duplicateQuiz(req: Request, res: Response) {
  const quiz = await Quiz.findById(req.params.id);
  if (!quiz) throw new ApiError(404, 'Quiz not found');
  if (quiz.creatorId.toString() !== req.user!.userId) {
    throw new ApiError(403, 'You do not own this quiz');
 ...
 * wrong (or that belong to their weak topics) from quizzes they've attempted.
 * No new AI generation is needed here — it reuses the existing question bank,
 * which keeps this fast and free of extra AI-service calls.
 */
export async function generatePracticeQuiz(req: Request, res: Response) {
  ...
  if (topics.length === 0) {
    throw new ApiError(400, 'Provide at least one weak topic to practice');
  }

  const attempts = await Attempt.find({ participantId: req.user!.userId });
  const quizIds = [...new Set(attempts.map((a) => a.quizId.toString()))];
  const quizzes = await Quiz.find({ _id: { $in: quizIds } });

  const topicSet = new Set(topics.map((t) => t.toLowerCase()));
  const pool = quizzes.flatMap((q) =>
    q.questions.filter((question) => topicSet.has((question.topic || 'General').toLowerCase()))
  );

  if (pool.length === 0) {
    throw new ApiError(404, 'No questions found for the selected topics yet');
  }

  // Shuffle and cap at 15 questions for a focused practice session.
  const shuffled = [...pool].sort(() => Math.random() - 0.5).slice(0, 15);

  const practiceQuiz = await Quiz.create({
    title: `Practice: ${topics.join(', ')}`,
    description: 'Targeted practice quiz generated from your weak topics.',
    creatorId: req.user!.userId,
    questions: shuffled,
    settings: {
  ...
  res.status(201).json({ quiz: practiceQuiz });
}

export async function getQuizByCode(req: Request, res: Response) {
  const quiz = await Quiz.findOne({ shareCode: req.params.code.toUpperCase() });
  if (!quiz || quiz.status !== 'published') {
    throw new ApiError(404, 'No published quiz found for this code');
  }
